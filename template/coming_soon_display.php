...
<?php if($general_settings['background']=="") { ?>  background:<?php if($design_settings['background_color']!='') { echo ($design_settings['background_color']); }
else { echo('#21759B'); } ?> <?php if($design_settings['background_effect']=="1") { ?> url(<?php echo plugins_url('/images/overlay/overlay-1.png', __FILE__); ?>) repeat <?php } ?>;   <?php } else {  ?>  background:url('<?php echo $general_settings['background']; ?>'); background-repeat: no-repeat; background-attachment: fixed; background-position: top center; -webkit-background-size: cover; -moz-background-size: cover; -o-background-size: cover; background-size: cover;  <?php }  ?>} 
#title_style {  
...
#descrip {  
 ...
 font-family:<?php echo $design_settings['description_font_format'] ?>; font-style:normal; line-height:1.5;  text-align:center;  width:80%;  word-break: break-word;  word-break break-word;  -ms-word-wrap: normal | break-word;
}
</style>
...
<div class="container" style="height:auto"> 
  ...
  <div class="hero-unit" style=" background:rgba(0,0,0,0.5);">
    <center>
      <div id="title_style" >
        <h1>
          <?php if($general_settings['title']!=''){echo ucwords($general_settings['title']); } else { echo('Maintainance Mode');} ?>
        </h1>
      </div>
      <div id="descrip" >
        <p>
          <?php if($general_settings['descrip']!=''){echo ($general_settings['descrip']); } else { echo('This website will launch soon. This a place-holder text can be edited through option panel. Also add email subscription box through option panel. ');} ?>
        </p>
      </div>
...
{?>
      <form id="subscribe" class="form-signin" action="#" method="post"  style=" width:80%;background:#2a6888; border:0px groove #333333;padding: 6px 5px 7px 5px; ">
        ...
        <?php if($notification_settings['sb_btn']!=''){echo ($notification_settings['sb_btn']); } else { echo('Submit');} ?>
        </button>
      </form>
      <?php } ?>
    </center>
